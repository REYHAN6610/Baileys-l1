...
![WhatsApp API Mod](https://raw.githubusercontent.com/REYHAN6610/GhostRosa/refs/heads/main/903c74ba-ce17-4a19-afc1-48dfa5298ac6.jpeg)
...
# Extra
🚀 Fast – Optimized for speed and smooth performance.
🔓 Open Source – Built with transparency and freedom in mind.
💪 Powerful – Enhanced features for advanced WhatsApp automation.

##⚠️ Warning:
This project is for educational and personal use only.
Do not sell, redistribute, or add malicious modifications.
...
```bash
npm install baileys-l1
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```bash
yarn add baileys-l1@latest
```

Then import your code using:
```javascript
const { default: makeWASocket } = require("baileys-l1")
```
...
- [Sending Messages](#sending-messages)
    - [Non-Media Messages](#non-media-messages)
        ...
        - [Buttons Product List](#buttons-product-list)
        - [Katalog Message](#katalog-message)
        - [Copy Pix](#copy-pix)
...
```javascript
const makeWASocket = require("baileys-l1").default;
const { useMultiFileAuthState } = require("baileys-l1");
...
```javascript
const makeWASocket = require("baileys-l1").default;
const { DisconnectReason, useMultiFileAuthState } = require("baileys-l1");
...
```javascript
const makeWASocket = require("baileys-l1").default;
const { makeInMemoryStore } = require("baileys-l1");
...
#### Sticker Message
```javascript
const stickerMessage = await generateWAMessageFromContent(jid, {
    stickerMessage: {
        url: fs.readFileSync('./path/to/sticker.webp'),
        mimetype: 'image/webp',
        isAnimated: false // true for gif media
    }
}, { userJid: sock.user.id });

await sock.relayMessage(jid, stickerMessage.message, {
...
const { downloadMediaMessage, getContentType } = require("baileys-l1");
